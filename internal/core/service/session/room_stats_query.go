...
func (roomStatsQuery) Build(rooms []entity.Room) sessionio.RuntimeStatsResponse {
	stats := sessionio.RuntimeStatsResponse{
		...
		ByRuntimeEvent: make(map[string]int),
	...
	for _, room := range rooms {
		...
		if room.LastRuntimeEventType != "" {
			stats.ByRuntimeEvent[room.LastRuntimeEventType]++
		}
		...
		stats.RoomsDetail = append(stats.RoomsDetail, sessionio.RuntimeRoomStatDetail{
			...
			LastRuntimeEventAt:   formatOptionalTime(room.LastRuntimeEventAt),
			Tracks:               trackCount,
		})
	}
